...
int main(void)
{
    ...
    // Define Input Layout - Count the number of floats per vertex, it is '5'
    ...
    float vertices[] =
    { //   x,     y,    z,    u,     v   // we can list the vertex number here for convenience
        0.0,  2.0f,  0.0f,   0.0,   0.0, // 0
        1.0f, 0.0f,  1.0f,   0.0,   1.0, // 1
        1.0f, 0.0f, -1.0f,   1.0,   0.0, // 2

        0.0,  2.0f,  0.0f,   0.0,   0.0, // 0
        1.0f, 0.0f, -1.0f,   1.0,   0.0, // 2
       -1.0f, 0.0f, -1.0f,   0.5,   0.5, // 4

       0.0,  2.0f,  0.0f,    0.0,   0.0, // 0
      -1.0f, 0.0f, -1.0f,    0.5,   0.5, // 4
      -1.0f, 0.0f,  1.0f,    1.0,   1.0, // 3
    ...
    UINT stride = 5 * sizeof(float); // see layout above, and make sure this matches
    ...
    ID3D11Buffer *pvscb;
    std::unique_ptr<VSConstBuffer> vscb_cpu = std::make_unique<VSConstBuffer>();
    
	// initialize the matrices
	vscb_cpu->m_world = XMMatrixTranspose(XMMatrixIdentity());
	XMVECTOR eye = XMVectorSet(0, 0, -5.f, 0);
	XMVECTOR at = XMVectorSet(0, 0, 0, 0);
	XMVECTOR up = XMVectorSet(0, 1.f, 0, 0);
    vscb_cpu->m_view = XMMatrixTranspose(XMMatrixLookAtLH(eye, at, up));
    vscb_cpu->m_proj = XMMatrixTranspose(XMMatrixPerspectiveFovLH(XM_PIDIV2, width / float(height), 0.1f, 100.f));

    D3D11_BUFFER_DESC vscb_bd = { 0 };
    vscb_bd.Usage = D3D11_USAGE_DEFAULT;
    vscb_bd.ByteWidth = sizeof(VSConstBuffer);
    vscb_bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    vscb_bd.CPUAccessFlags = 0;
    hr = dxDevice->CreateBuffer(&vscb_bd, nullptr, &pvscb);
    MYASSERT(SUCCEEDED(hr));
    ...
    while(isRunning)
    {
        ...
        // update VS const buffer
        t += float(XM_PI) * 0.01f;
        vscb_cpu->m_world = XMMatrixTranspose(XMMatrixRotationY(t));
        dxDeviceContext->UpdateSubresource(pvscb, 0, nullptr, vscb_cpu.get(), 0, 0);
        ...
        dxDeviceContext->VSSetConstantBuffers(0, 1, &pvscb);
